...
(** [generate_inf_pairs] is infinite sequence of infinite sequences of 
    integer pairs ([a], [b]) such that [a] < [b] *)
...
(** [generate_triples_up_to_sum sum] if finite sequence of 
    integer triples ([a], [b], [c]) such that [a] < [b] < [c]
    limited by ([a]+[b]+[c]) = [sum] *)
